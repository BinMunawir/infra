...
log "status:   just status"
log "verify:   just verify l1"
...
log ""
log "NOTE: this layer owns the edge, so ${LB_ACTUAL} should answer once the"
log "      'edge' Application is Healthy — with a 404, until L2 adds routes."
log "      istiod assigns the gateway's NodePort dynamically and the Gateway API"
log "      cannot pin it, so one imperative step remains:  just edge-port"
